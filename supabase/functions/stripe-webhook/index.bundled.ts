...
function buildAdminOrderHtml(order: Record<string, string>): string {
  return `<!DOCTYPE html>
<html>
...
function buildCustomerOrderHtml(order: Record<string, string>): string {
  return `<!DOCTYPE html>
<html>
<head><meta charset="utf-8"></head>
<body style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      if (orderError) {
        ...
        const unitLabel = qty === 1 ? "ejemplar" : "ejemplares";
        const { error: saleError } = await supabase.from("donations").insert({
          amount: parseFloat(meta.total_amount),
          date: new Date().toISOString().split("T")[0],
          donor_name: null,
          description: `Venta de ${qty} ${unitLabel} de "${meta.product_name}"`,
        });
        if (saleError) {
          console.error("Error inserting donation from sale:", JSON.stringify(saleError));
        } else {
          console.log("Donation from sale saved:", qty, meta.product_name);
        }
      }

      const order = meta as Record<string, string>;
      ...
      try {
        const { data: settings, error: settingsError } = await supabase
          ...
          .limit(1)
          .maybeSingle();
        if (settingsError) console.error("Error fetching shop_settings:", JSON.stringify(settingsError));
        const notificationEmail = settings?.notification_email;
        console.log("shop_settings query result - notification_email:", notificationEmail ?? "(null/empty)");
        if (notificationEmail) {
          ...
          console.log("No notification_email in shop_settings, skipping admin email");
        }
      } catch (adminEmailErr) {
